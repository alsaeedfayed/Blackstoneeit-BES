...
@Component({
  selector: 'app-update-progress-model',
  templateUrl: './update-progress-model.component.html',
  styleUrls: ['./update-progress-model.component.scss']
})
export class UpdateProgressModelComponent implements OnInit {
  ...
  updateProgress() {
    ...
    this.httpSer.post(Config.Task.UpdateProgress, body)
      .pipe(finalize(() => { this.isUpdatingTask = false; }))
      .subscribe(() => {
        this.toastr.success(this.translate.instant('committeeTasks.updateProgress.updateProgressSuccessMsg'));
        this.onBack.emit();
      })
  }
