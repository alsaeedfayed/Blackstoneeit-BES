...
@Component({
  selector: 'app-evaluation-calender',
  templateUrl: './evaluation-calender.component.html',
  styleUrls: ['./evaluation-calender.component.scss']
})
export class EvaluationCalenderComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.list.forEach(evaluation => {
      let calendarMeeting = {
        id: evaluation.id,
        title: evaluation.typeName,
        description: '',
        //location: meeting.locationType,
        start: this.getTimeWithCorrectDate(evaluation.updatedDate, evaluation.from),
        end: this.getTimeWithCorrectDate(evaluation.updatedDate, evaluation.to),

        color: this.meetingStatusColors[evaluation.status]['color'],
