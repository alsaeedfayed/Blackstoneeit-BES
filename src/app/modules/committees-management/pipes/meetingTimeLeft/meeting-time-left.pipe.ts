...
@Pipe({
  name: 'meetingTimeLeft'
})
export class MeetingTimeLeftPipe implements PipeTransform {
  constructor() {
    registerLocaleData(localeAr);
  }

  transform(meetingDate: string, startTime: string, endTime: string, language: string): string {
    let now = new Date();
    let date = this.convertUTCDateToLocalDate(meetingDate);
