...
export class NgxPermissionsDirective implements OnInit, OnDestroy, OnChanges {
  @Input() ngxPermissionsOnly: any | any[];
      ...
      .then(([hasPermission, hasRole]) => {
        if (hasPermission || hasRole) {
          this.handleUnauthorisedPermission(
            this.ngxPermissionsExceptElse || this.ngxPermissionsElse
          );
          return;
        }
    ...
    Promise.all([
      ...
      .then(([hasPermissions, hasRoles]) => {
        if (hasPermissions || hasRoles) {
          ...
          this.handleUnauthorisedPermission(
            this.ngxPermissionsOnlyElse || this.ngxPermissionsElse
          );
        }
      })
      .catch(() => {
  ...
  private handleUnauthorisedPermission(template: TemplateRef<any>): void {
    if (
      isBoolean(this.currentAuthorizedState) &&
  ...
  private getAuthorizedStrategyInput() {
    return (
      this.ngxPermissionsOnlyAuthorisedStrategy ||
  ...
  private getUnAuthorizedStrategyInput() {
    return (
      this.ngxPermissionsOnlyUnauthorisedStrategy ||
